(function(){
  ...
  shell.innerHTML='<button class="home-mobile-menu" type="button" aria-label="打开花园导航" aria-expanded="false">'+icon('<path d="M4 7h16M4 12h16M4 17h16"/>')+'</button><div class="home-nav-backdrop"></div>'+ 
    '<aside class="home-sidebar" aria-label="花园导航"><div class="home-sidebar-inner"><div class="home-nav-group"><p class="home-nav-label">花园导航</p>'+navItem('featured','精选','', '<path d="m12 3 2.2 4.5L19 8.2l-3.5 3.4.8 4.8-4.3-2.3-4.3 2.3.8-4.8L5 8.2l4.8-.7L12 3z"/>')+navItem('all','全部专题','7','<rect x="4" y="4" width="6" height="6" rx="1"/><rect x="14" y="4" width="6" height="6" rx="1"/><rect x="4" y="14" width="6" height="6" rx="1"/><rect x="14" y="14" width="6" height="6" rx="1"/>')+'</div>'+ 
    '<div class="home-nav-group"><p class="home-nav-label">知识分类</p>'+navItem('tech','技术','3','<circle cx="12" cy="12" r="3"/><path d="M12 3v3M12 18v3M3 12h3M18 12h3M5.6 5.6l2.1 2.1M16.3 16.3l2.1 2.1M18.4 5.6l-2.1 2.1M7.7 16.3l-2.1 2.1"/>')+navItem('think','思维','4','<path d="M9 18h6M10 21h4M12 3a7 7 0 0 0-4 12.7c.6.5 1 1.2 1 2V18h6v-.3c0-.8.4-1.5 1-2A7 7 0 0 0 12 3z"/>')+'</div>'+ 
    ...
    '<main class="home-main" id="top"><section class="home-hero-new"><div class="home-hero-copy"><p class="home-kicker">The Knowledge Garden</p><h1>StoryGarden</h1><p class="home-lead">把每一次深潜、每一页笔记，种成一片会生长的花园</p><button class="home-search" type="button" data-sg-open-search>'+icon('<circle cx="11" cy="11" r="7"/><path d="m20 20-4-4"/>')+'<span>搜索专题、章节或关键词……</span><kbd>Ctrl K</kbd></button><p class="home-meta">7 个专题 · 100+ 知识页面 · 最近照料于 2026-08-28</p></div></section>'+ 
    '<section class="home-library" id="home-library"><div class="home-library-head"><div class="home-library-title"><p data-home-label>Curated topics</p><h2 data-home-heading>精选专题</h2></div><div class="home-library-status"><strong data-home-count>7</strong> 个专题正在生长</div></div><div class="home-topic-grid">'+topics.map(renderCard).join('')+'</div></section></main>'+ 
  ...
  var labels={featured:['Curated topics','精选专题'],all:['All topics','全部专题'],tech:['Technology','技术专题'],think:['Thinking','思维专题'],recent:['Recently tended','最近更新']};
  ...
  function setFilter(filter){
    ...
    if(filter==='recent')ordered.sort(function(a,b){return b.getAttribute('data-date').localeCompare(a.getAttribute('data-date'));});
    else ordered.sort(function(a,b){return Number(a.getAttribute('data-order'))-Number(b.getAttribute('data-order'));});
    ordered.forEach(function(card){
      grid.appendChild(card);
      var visible=filter==='featured'||filter==='all'||filter==='recent'||card.getAttribute('data-category')===filter;
      card.hidden=!visible;
      var order=card.getAttribute('data-order');
      card.classList.toggle('is-featured',visible&&filter==='featured'&&(order==='1'||order==='2'));
    });
